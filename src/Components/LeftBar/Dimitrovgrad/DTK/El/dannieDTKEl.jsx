...
var Choosen;
class DannieDTKEl extends React.Component{
    constructor() {
        super()
        this.state = {
            El: []
    ...
    componentDidMount() {
        ...
        const ElMeh = db.collection("Город").doc("Димитровград").collection("ДТК").doc("Специальность").collection("Автоэлектрик");
        let getEl = ElMeh.get()
        .then(snapshot => {
    ...
    return (
        <div>
            {this.state.El.map(item => (
          <NavLink to={"/dimitrovgrad/DTK/El/Choosen/"+item.id} onClick={this.knop =()=> {Choosen=item.id; console.log(Choosen)}}><div className="block">
