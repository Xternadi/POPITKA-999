...
var Choosen;
class DannieMolHolod extends React.Component{
    constructor() {
        super()
        this.state = {
            Holod: []
    ...
    componentDidMount() {
        ...
        const HolodMeh = db.collection("Город").doc("Димитровград").collection("Молочка").doc("Специальность").collection("Криотехник");
        let getHolod = HolodMeh.get()
        .then(snapshot => {
    ...
    return (
        <div>
            {this.state.Holod.map(item => (
          <NavLink to={"/dimitrovgrad/Mol/Holod/Choosen/"+item.id} onClick={this.knop =()=> {Choosen=item.id; console.log(Choosen)}}><div className="block">
