...
var Choosen;
class DannieMolBank extends React.Component{
    constructor() {
        super()
        this.state = {
            Bank: []
    ...
    componentDidMount() {
        ...
        const BankMeh = db.collection("Город").doc("Димитровград").collection("Молочка").doc("Специальность").collection("Банковское дело");
        let getBank = BankMeh.get()
        .then(snapshot => {
    ...
    return (
        <div>
          ...
          <NavLink to={"/dimitrovgrad/Mol/Bank/Choosen/"+item.id} onClick={this.knop =()=> {Choosen=item.id; console.log(Choosen)}}><div className="block">
